 ...
 async function createSessionCookie(idToken: string): Promise<boolean> {
   ...
   if (!response.ok) {
     try {
       ...
       console.warn('createSessionCookie falhou (register):', response.status, body);
     } catch (e) {
 ...
 export default function RegisterPage() {
   ...
   const handleRegister = async (e: React.FormEvent) => {
     ...
     try {
       ...
       if (!sessionCreated) throw new Error('Falha ao criar sessão no servidor.');
   ...
   const handleGoogleSignUp = async () => {
     ...
     try {
       ...
       const sessionCreated = await createSessionCookie(idToken);
       if (!sessionCreated) throw new Error('Falha ao criar sessão do Google no servidor.');
